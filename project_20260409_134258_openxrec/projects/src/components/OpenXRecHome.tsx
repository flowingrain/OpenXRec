...
export default function OpenXRecHome() {
  ...
  // 每个推荐项的反馈状态：{ itemId: 'like' | 'dislike' }
  const [itemFeedbacks, setItemFeedbacks] = useState<Record<string, 'like' | 'dislike'>>({});
  // 知识库面板状态
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/recommendation', {
        ...
        body: JSON.stringify({
          query: input,  // 顶级 query 字段
          userId: sessionId,
          scenario: 'general',
          sessionContext: requestContext,  // 传递会话上下文
          options: {
      ...
      if (data.success && data.data?.metadata?.clarification?.needsClarification) {
        ...
        const assistantMessage: Message = {
          ...
          content: isComparisonAnalysis 
            ? `为您生成对比分析：`
            : isRanking 
              ? `为您综合分析出 ${recommendations.length} 个推荐选项：`
              : `为您找到 ${recommendations.length} 个推荐结果：`,
          timestamp: Date.now(),
          recommendationType,
                      ...
                      {messages.length === 0 ? (
                          <div className="text-center py-8">
                            ...
                            <p className="text-muted-foreground mb-4">
                              您好！我是 OpenXRec 推荐助手。请描述您的需求，我会为您提供可解释的个性化推荐。
                            </p>
                                    ...
                                    >
                                      ...
                                      <div className="mt-3 flex gap-2">
                                        <Button
                                          ...
                                          disabled={!!itemFeedbacks[rec.id]}
                                          onClick={() => sendFeedback(rec.id, 'like')}
                                        >
                                          <ThumbsUp className={`w-3 h-3 mr-1 ${itemFeedbacks[rec.id] === 'like' ? 'fill-green-500 text-green-500' : ''}`} />
                                          {itemFeedbacks[rec.id] === 'like' ? '已反馈' : '有帮助'}
                                        </Button>
                                        <Button
                                          variant="ghost"
                                          size="sm"
                                        ...
                                        >
                                          <ThumbsDown className={`w-3 h-3 mr-1 ${itemFeedbacks[rec.id] === 'dislike' ? 'fill-red-500 text-red-500' : ''}`} />
                                          {itemFeedbacks[rec.id] === 'dislike' ? '已反馈' : '不相关'}
                                        </Button>
                                      </div>
                                    </div>
                                  ))}
