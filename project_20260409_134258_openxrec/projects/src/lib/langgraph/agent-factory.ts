...
import { DomainAdapterManager, domainManager, DomainType, ScenarioType, AgentLayer } from './domain-config';
import {
  coordinatorNode,
  searchNode,
  sourceEvaluatorNode,
  ...
  getWorkflowConfig(): {
    ...
    return {
      ...
      executionStrategy: scenario.executionStrategy || {
        layers: ['perception', 'cognition', 'decision'] as AgentLayer[],
        skipAgents: [],
        parallelWithinLayer: false,
        maxIterations: 10
      },
