...
// 重新导出 AgentLayer
export type { AgentLayer } from './state';
import { AgentLayer } from './state';
...
export interface DomainIndicator {
  ...
  formula?: string;  // 计算公式
  thresholds?: { low: number; medium: number; high: number; critical?: number };  // 阈值
  direction?: 'higher_is_risk' | 'lower_is_risk';  // 方向
  weight?: number;  // 权重
...
export interface ScenarioConfig {
  ...
  // 扩展字段（向后兼容）
  domainType?: string;
  agentLayers?: { required: string[]; optional: string[] };
