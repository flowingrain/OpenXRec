    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return candidates.map(c => {
          const features = featureMap.get(c.id) as { features?: Record<string, any>; relevanceScore?: number; matchReasons?: string[] } | undefined;
          return {
            ...c,
...
返回JSON：
{
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return items.map(item => {
          const ranking = rankingMap.get(item.id) as { score?: number; confidence?: number; rankingReasons?: string[] } | undefined;
          const typedItem = item as { relevanceScore?: number; matchReasons?: string[] };
          return {
            ...item,
            score: ranking?.score ?? typedItem.relevanceScore ?? 0.7,
            confidence: ranking?.confidence || 0.7,
            matchReasons: ranking?.rankingReasons || typedItem.matchReasons || [],
            // 新增：融合额外信息
  ...
  private async generateSingleExplanation(
    ...
    // 分析用户画像特征
    const isBeginner = /零基础|入门|新手|初学|不会/i.test(query);
    const isProfessional = /进阶|高级|专业|深入/i.test(query);
    const hasBudget = /预算|\d+元|价格|费用|钱/i.test(query);
    const isEnterprise = /企业|公司|团队|机构/i.test(query);
    
    const userProfile = isBeginner ? '新手用户' : 
                        isProfessional ? '进阶用户' : 
                        isEnterprise ? '企业用户' : '普通用户';
    
    const prompt = `你是OpenXRec的解释生成智能体。为以下推荐生成**从用户视角**的推荐理由。
...
## 你的用户画像
${userProfile}
${isBeginner ? '（你需要的是简单易懂的解释，避免专业术语）' : ''}
${isProfessional ? '（你需要深入的技术分析和专业建议）' : ''}
${hasBudget ? '（你需要关注性价比和成本）' : ''}
${isEnterprise ? '（你需要关注企业级功能和合规性）' : ''}

## 识别到的用户需求
${intentResult.userIntent || '从查询中提取的核心需求'}

## 推荐项目（需要给用户推荐的理由）
"${item.title}"
${item.description ? `简要描述：${item.description.substring(0, 100)}...` : ''}
...
✓ 【你的核心需求1】→ 【物品如何满足】
✓ 【你的核心需求2】→ 【物品如何满足】
${hasBudget ? '💰 相比同类：这个在【价格/性价比】上更有优势' : '✨ 相比同类：在【某个维度】更突出' }"

**关键要求**：
1. **直接称呼"你"**，语气友好自然
2. **引用用户查询中的关键词**
3. **区分用户水平**：对新手用通俗语言，对专业用户可加技术细节
4. **加入对比信息**：说明相比其他推荐项的独特优势
5. 控制在50-70字

**输出格式**（JSON）：
{
  "type": "feature_similarity|behavioral|causal|knowledge_graph|collaborative|rule_based",
  "reason": "【从用户视角的推荐理由，50-70字】",
  "factors": [
    {"name": "你的需求", "value": "【用户查询中的具体需求】", "importance": 0.95, "category": "user"},
    {"name": "匹配点", "value": "【物品如何满足，用动词开头】", "importance": 0.85, "category": "item"},
    {"name": "相比优势", "value": "【相比其他推荐项的独特优势】", "importance": 0.75, "category": "comparison"}
  ]
}`;
  ...
  private generateOverallExplanation(
    ...
    const scenario = scenarioText[intentResult.scenarioType] || '综合分析';

    return `基于${scenario}场景分析，为您推荐${items.length}个选项。首选"${topItem.title}"，${topItem.explanations[0]?.reason || '综合评估最优'}`;
  }
          ...
          chat: async (params) => {
            const response = await this.llmClient.invoke(
              [
                ...
                ...params.messages,
              ] as Parameters<typeof this.llmClient.invoke>[0],
              {
                model: 'doubao-seed-2-0-pro-260215',
                temperature: 0.5,
              }
...
请分析当前推荐策略的效果，返回JSON：
{
  ...
  "reasoning": "基于分析结果，建议调整权重的原因"
    ...
    try {
      ...
      // 使用括号平衡算法找到正确的 JSON 结束位置
      const findJsonEnd = (str: string): string => {
        let braceCount = 0;
        let bracketCount = 0;
        let inString = false;
        let escaped = false;
      ...
      if (jsonMatch) {
        jsonStr = findJsonEnd(jsonMatch[0]);
      }
      ...
      try {
        ...
        console.warn('[ConfigOptimizer] JSON parse failed after cleanup');
        console.log('[ConfigOptimizer] Raw response length:', response.content.length);
      }
    } catch (e) {
