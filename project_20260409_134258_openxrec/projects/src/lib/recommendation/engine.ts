  ...
  async recommend(request: RecommendationRequest): Promise<RecommendationResult[]> {
    ...
    switch (effectiveStrategy) {
      ...
      case 'knowledge_based':
        ...
        break;
      case 'agent_based':
        scoredItems = await this.agentBasedScoring(candidates, userProfile, context || { time: Date.now(), query: '' });
        break;
      case 'causal_based':
        scoredItems = await this.causalBasedScoring(candidates, userProfile, context || { time: Date.now(), query: '' });
        break;
      default:
        scoredItems = await this.hybridScoring(candidates, userProfile);
    }
