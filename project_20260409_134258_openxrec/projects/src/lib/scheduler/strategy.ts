...
export class CapabilityMatchStrategy implements ISchedulingStrategy {
  ...
  selectAgent(task: Task, availableAgents: Agent[]): Agent | null {
    ...
    // 计算每个智能体的能力匹配度
    const taskCaps = task.requiredCapabilities as AgentCapability[];
    const scoredAgents = availableAgents.map(agent => {
      const matchedCapabilities = taskCaps.filter(
        cap => agent.capabilities.includes(cap)
      );
      const matchScore = matchedCapabilities.length / taskCaps.length;
      const extraCapabilities = agent.capabilities.filter(
        cap => !taskCaps.includes(cap)
      );
      // 偏好匹配度高但不多余的智能体
...
export class HybridStrategy implements ISchedulingStrategy {
  ...
  selectAgent(task: Task, availableAgents: Agent[]): Agent | null {
    ...
    const scoredAgents = availableAgents.map(agent => {
      ...
      // 能力匹配分数
      const requiredCaps = (task.requiredCapabilities || []) as AgentCapability[];
      if (requiredCaps.every(cap => agent.capabilities.includes(cap))) {
        capabilityScore = 1;
      }
