...
export class SupabaseVectorStore {
  ...
  async search(
    ...
    if (embeddingType) {
      queryBuilder = queryBuilder.eq('embeddingType', embeddingType);
    }
      ...
      if (similarity >= threshold) {
        results.push({
          entry: {
            ...
            embeddingType: embeddingRecord.embeddingType,
            embedding: embedding,
  ...
  async getEntriesByType(
    ...
    const { data, error } = await this.supabase
      .from('case_embeddings')
      .select('*')
      .eq('embeddingType', embeddingType)
      .limit(limit);
  ...
  async updateEntry(
    ...
    if (updates.embeddingType) {
      updateData.embeddingType = updates.embeddingType;
    }
  ...
  async getStats(): Promise<VectorStoreStats> {
    ...
    for (const item of data) {
      byType[item.embeddingType] = (byType[item.embeddingType] || 0) + 1;
  ...
  async getEmbeddingTypes(): Promise<string[]> {
    const { data, error } = await this.supabase
      .from('case_embeddings')
      .select('embeddingType');

    if (error || !data) {
      return [];
    }

    const types = [...new Set(data.map(item => item.embeddingType))];
    return types;
  }
  ...
  async exportData(options: {
    ...
    if (options.embeddingType) {
      query = query.eq('embeddingType', options.embeddingType);
    }
    ...
    const embeddingTypes = [...new Set(data.map(item => item.embeddingType))];
  ...
  async clearAll(options: {
    ...
    const deleted = options.embeddingType
      ? (stats.byType[options.embeddingType] || 0)
      : stats.total;
