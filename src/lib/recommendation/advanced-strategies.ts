...
export class MultiBehaviorRecommender {
  ...
  calculateBehaviorScore(behavior: any): number {
    let score = this.behaviorWeights[behavior.action] || 0;
  ...
  learnWeights(feedback: Array<{
    ...
    for (const f of feedback) {
      const action = f.behavior.action;
    ...
    for (const key in newWeights) {
      newWeights[key] = Math.abs(newWeights[key]) / total;
    }
...
export type { SequenceWindow, BehaviorWeights };
